# coding=utf-8

import copy
from functools import reduce
import config_util
import time

SboxTable = [
    ,
]

FK = [0xa3b1bac6, 0x56aa3350, 0x677d9197, 0xb27022dc]

CK = [
    0x00070e15, 0x1c232a31, 0x383f464d, 0x545b6269,
    0x70777e85, 0x8c939aa1, 0xa8afb6bd, 0xc4cbd2d9,
    0xe0e7eef5, 0xfc030a11, 0x181f262d, 0x343b4249,
    0x50575e65, 0x6c737a81, 0x888f969d, 0xa4abb2b9,
    0xc0c7ced5, 0xdce3eaf1, 0xf8ff060d, 0x141b2229,
    0x30373e45, 0x4c535a61, 0x686f767d, 0x848b9299,
    0xa0a7aeb5, 0xbcc3cad1, 0xd8dfe6ed, 0xf4fb0209,
    0x10171e25, 0x2c333a41, 0x484f565d, 0x646b7279
]

ENCRYPT = 0
DECRYPT = 1


def GET_UINT32_BE(key_data):
    return int((key_data[0] << 24) | (key_data[1] << 16) | (key_data[2] << 8) | (key_data[3]))


def PUT_UINT32_BE(n):
    return [int((n >> 24) & 0xff), int((n >> 16) & 0xff), int((n >> 8) & 0xff), int((n) & 0xff)]


def SHL(x, n):
    return int(int(x << n) & 0xffffffff)


def ROTL(x, n):
    return SHL(x, n) | int((x >> (32 - n)) & 0xffffffff)


def XOR(a, b):
    return list(map(lambda x, y: x ^ y, a, b))


def sm4Sbox(idx):
    return SboxTable[idx]


def sm4CalciRK(ka):
    a = PUT_UINT32_BE(ka)
    b = [sm4Sbox(i) for i in a]
    bb = GET_UINT32_BE(b)
    rk = bb ^ (ROTL(bb, 13)) ^ (ROTL(bb, 23))
    return rk


def sm4Lt(ka):
    a = PUT_UINT32_BE(ka)
    b = [sm4Sbox(i) for i in a]
    bb = GET_UINT32_BE(b)
    return bb ^ (ROTL(bb, 2)) ^ (ROTL(bb, 10)) ^ (ROTL(bb, 18)) ^ (ROTL(bb, 24))


def sm4F(x0, x1, x2, x3, rk):
    return (x0 ^ sm4Lt(x1 ^ x2 ^ x3 ^ rk))


class Sm4(object):
    def __init__(self):
        self.sk = [0] * 32
        self.mode = ENCRYPT

    def sm4_set_key(self, key_data, mode):
        self.sm4_setkey(key_data, mode)

    def sm4_setkey(self, key, mode):
        MK = [0, 0, 0, 0]
        k = [0] * 36
        MK[0] = GET_UINT32_BE(key[0:4])
        MK[1] = GET_UINT32_BE(key[4:8])
        MK[2] = GET_UINT32_BE(key[8:12])
        MK[3] = GET_UINT32_BE(key[12:16])
        k[0:4] = XOR(MK, FK)
        for i in range(32):
            k[i + 4] = k[i] ^ (sm4CalciRK(k[i + 1] ^ k[i + 2] ^ k[i + 3] ^ CK[i]))
        self.sk = k[4:]
        self.mode = mode
        if mode == DECRYPT:
            self.sk.reverse()

    def sm4_one_round(self, sk, in_put):
        item = [GET_UINT32_BE(in_put[0:4]), GET_UINT32_BE(in_put[4:8]), GET_UINT32_BE(in_put[8:12]),
                GET_UINT32_BE(in_put[12:16])]
        res = reduce(lambda x, y: [x[1], x[2], x[3], sm4F(x[0], x[1], x[2], x[3], y)], sk, item)
        res.reverse()
        rev = map(PUT_UINT32_BE, res)
        out_put = []
        [out_put.extend(_) for _ in rev]
        return out_put

    def sm4_crypt_ecb(self, input_data, mode=ENCRYPT):
        # SM4-ECB block encryption/decryption

        # pad
        if mode == ENCRYPT:
            input_data = self.sm4_pad_data(input_data)
        elif mode == DECRYPT:
            self.sk.reverse()

        if config_util.CRYPTO_MODE_DEBUG_OUTPUT:
            print("SM4 加解密文大小：%d 字节" % len(input_data))
            begin_time = time.time()*1000
            print("开始时间：%d" % begin_time)

        output_data = []
        tmp = [input_data[i:i + 16] for i in range(0, len(input_data), 16)]
        [output_data.extend(each) for each in map(lambda x: self.sm4_one_round(self.sk, x), tmp)]

        # unpad
        if mode == DECRYPT:
            output_data = self.sm4_unpad_data(output_data)
            self.sk.reverse()

        if config_util.CRYPTO_MODE_DEBUG_OUTPUT:
            end_time = time.time()*1000
            print("结束时间：%d" % end_time)
            print("SM4 用时:" + str(end_time - begin_time) + "毫秒")

        return output_data

    # 在加密之前padding
    def sm4_pad_data(self, input_data):
        length = len(input_data)
        input_data.append(0x80)
        for i in range((length+1)%16, 16):
            input_data.append(0x0)

        return input_data

    # 在解密后unpadding
    def sm4_unpad_data(self, data):
        # 若密钥错误可能全部pop出去，len(data) == 0
        while data[-1] != 0x80 and len(data) > 0:
            data.pop()
        data.pop()
        return data

    def print_selfkey(self):
        print(self.sk)


'''
用法：
key = 16 Byte list
data = list

sm4_d = Sm4()
sm4_d.sm4_set_key(key_data, ENCRYPT)
en_data = sm4_d.sm4_crypt_ecb(file_data_list)
'''

if __name__ == "__main__":
    # log_init()

    # file test
    file_path = r"C:\Users\肖\Desktop\各式语言\python\密码学final\dist\GUI.exe"
    ecb_path_en = r"test2k_ecb_en.py"
    ecb_path_de = r"test2k_ecb_de.py"
    cbc_path_en = r"test2k_cbc_en.py"
    cbc_path_de = r"test2k_cbc_de.py"

    key_data = [0x01, 0x02, 0x03, 0x04, 0x05, 0x06, 0x07, 0x08,
                0x01, 0x02, 0x03, 0x04, 0x05, 0x06, 0x07, 0x08]
    iv_data = [0x5a] * 16

    with open(file_path, 'rb') as f:
        file_data = f.read()
    # file_data_list = [ord(x) for x in file_data]
    file_data_list = list(file_data)
    file_data_list_bak = file_data_list[:]
    # 1. ECB
    sm4_d = Sm4()
    sm4_d.sm4_set_key(key_data, ENCRYPT)

    sm4_d.print_selfkey()

    en_data = sm4_d.sm4_crypt_ecb(file_data_list, mode=ENCRYPT)
    with open(ecb_path_en, 'w+b') as f:
        f.write(bytes(en_data))

    sm4_d.sm4_set_key(key_data, DECRYPT)
    de_data = sm4_d.sm4_crypt_ecb(en_data, mode=DECRYPT)
    with open(ecb_path_de, 'w+b') as f:
        f.write(bytes(de_data))
    if de_data == file_data_list_bak:
        print("file decode pass")
    else:
        print("file decode fail")
        raise BaseException('error')
